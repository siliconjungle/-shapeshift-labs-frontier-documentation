# @shapeshift-labs/frontier-documentation

Frontier-native documentation manifests, generated documentation books, Vite virtual modules, and agent-readable evidence plans for Frontier apps and packages.

This package is its own documentation system. It does not implement compatibility layers for external documentation frameworks.

## Install

```sh
npm install @shapeshift-labs/frontier-documentation
```

## Use

```sh
npx frontier-docs discover --json
npx frontier-docs build
```

`frontier-docs build` writes a standalone documentation book to `.frontier/docs`:
...
const page = defineDocumentationPage({
  ...
  kind: 'guide',
  sections: [{ title: 'Install', content: 'npm install my-app' }]
});

export const docs = createDocumentationManifest({
  package: '@example/app',
  pages: [page]
});
```
...
- `@shapeshift-labs/frontier-documentation`: manifest types, validation, search, registry graph, proof, JSONL.
- `@shapeshift-labs/frontier-documentation/node`: source discovery and artifact writing.
- `@shapeshift-labs/frontier-documentation/browser`: standalone documentation book rendering and inspector bridge.
- `@shapeshift-labs/frontier-documentation/vite`: Vite virtual modules for framework/app integration.
- `@shapeshift-labs/frontier-documentation/harness`: lint, fuzz, benchmark, browser evidence, and agent runbook plans.
- `@shapeshift-labs/frontier-documentation/cli`: programmatic CLI entrypoint.

## Vite

```ts
import { frontierDocumentationVite } from '@shapeshift-labs/frontier-documentation/vite';

export default {
  plugins: [frontierDocumentationVite()]
};
```

Virtual modules:

- `virtual:frontier-documentation/manifest`
- `virtual:frontier-documentation/book`
- `virtual:frontier-documentation/client`
- `virtual:frontier-documentation/module`
- `virtual:frontier-documentation/search`

## Agent-First Defaults

All integrations default on but can be disabled:

```sh
frontier-docs build --no-telemetry --no-fuzz
frontier-docs discover --disable browser-evidence,benchmarks
```

Default integrations include package catalog discovery, API reference extraction, guide books, browser books, inspector bridge, component-preview links, route/state/migration docs, telemetry expectations, browser evidence, fuzz plans, benchmark plans, search indexes, and agent evidence.
...
The harness output is serializable and meant to feed Frontier test, Playwright evidence, CI gates, and agent workflows without importing those packages directly.

## Benchmarks

Run the Frontier-only package measurements with:

```sh
npm run bench
```

The benchmark writes `benchmarks/results/frontier-documentation-package-bench-latest.json` and measures manifest normalization, search index generation, harness generation, standalone book rendering, fuzz case generation, benchmark plan generation, and proof hashing.

Repository: https://github.com/siliconjungle/-shapeshift-labs-frontier-documentation
